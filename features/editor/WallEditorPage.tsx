...
export const WallEditor: React.FC<WallEditorProps> = ({ 
  ...
  const wrappedSaveCloud = async () => {
      ...
      if (success) {
          state.setIsDirty(false); // RÉSOLUTION DU BUG DE PERSISTANCE
      }
      return success;
  };
  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-black overflow-hidden font-sans">
      <SEO title={initialMode === 'VIEW' ? "Visualisation" : "Atelier de Création"} />
      ...
      <div className="flex flex-1 overflow-hidden relative">
        <div className={`relative z-20 h-full bg-gray-900 border-r border-gray-800 transition-all duration-300 ease-in-out flex flex-col ${isSidebarOpen ? 'w-80' : 'w-0 -translate-x-full opacity-0'}`}>
             ...
             <div className="flex-1 w-80 overflow-hidden relative">
                {initialMode === 'VIEW' ? (
                    ...
                    <EditorPanel config={config} holds={holds} onUpdate={setConfig} metadata={metadata} onNext={() => setActiveTab('holds')} showModal={(c) => state.setModal(c)} onActionStart={logic.saveToHistory} onExport={() => logic.handleAction('save')} onImport={logic.handleImportFile} onNew={logic.handleNewWallRequest} onHome={() => logic.handleAction('exit')} onRemoveSegment={logic.removeSegmentAction} />
                ) : (
                    <RouteEditorPanel onBack={() => setActiveTab('structure')} selectedHold={state.selectedHold} onSelectHold={state.setSelectedHold} metadata={metadata} holdSettings={state.holdSettings} onUpdateSettings={(s:any) => state.setHoldSettings(prev => ({ ...prev, ...s }))} placedHolds={holds} onRemoveHold={(id) => logic.removeHoldsAction([id], true)} onRemoveAllHolds={logic.handleRemoveAllHolds} onChangeAllHoldsColor={logic.handleChangeAllHoldsColor} selectedPlacedHoldIds={state.selectedPlacedHoldIds} onUpdatePlacedHold={(ids, u) => { const s = new Set(ids); setHolds(h => h.map(x => (x && s.has(x.id)) ? { ...x, ...u } : x)); state.setIsDirty(true); }} onSelectPlacedHold={state.handleSelectPlacedHold} onDeselect={() => state.setSelectedPlacedHoldIds([])} onActionStart={logic.saveToHistory} onReplaceHold={(ids, def) => { logic.saveToHistory(); const s = new Set(ids); setHolds(prev => prev.map(h => (h && s.has(h.id)) ? { ...h, modelId: def.id, filename: def.filename } : h)); }} onRemoveMultiple={() => logic.removeHoldsAction(state.selectedPlacedHoldIds, true)} onExport={() => logic.handleAction('save')} onImport={logic.handleImportFile} onNew={logic.handleNewWallRequest} onHome={() => logic.handleAction('exit')} />
                )}
             </div>
        </div>
        <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className={`absolute top-1/2 z-30 -translate-y-1/2 bg-gray-800 border border-gray-600 text-gray-400 p-1 rounded-r-lg hover:bg-blue-600 ${isSidebarOpen ? 'left-80' : 'left-0'}`}> {isSidebarOpen ? <ChevronLeft size={16} /> : <ChevronRight size={16} />} </button>
        <div className="flex-1 relative h-full bg-black">
            {initialMode !== 'VIEW' && (
                <div className="fixed bottom-6 right-6 z-[100] flex gap-2 p-1 bg-gray-950/80 backdrop-blur-md rounded-2xl border border-white/5 shadow-2xl">
                    <button disabled={!canUndo} onClick={logic.performUndo} className="p-3 hover:bg-white/10 rounded-xl text-white disabled:opacity-30"><Undo2 size={20} /></button>
                    <button disabled={!canRedo} onClick={logic.performRedo} className="p-3 hover:bg-white/10 rounded-xl text-white disabled:opacity-30"><Redo2 size={20} /></button>
                </div>
            )}
            <Scene config={config} mode={derivedMode} holds={holds.map(h => h && resolveHoldWorldData(h, config) ? {...h, ...resolveHoldWorldData(h, config)} : null).filter(Boolean) as any} onPlaceHold={logic.handlePlaceHold} selectedHoldDef={state.selectedHold} holdSettings={state.holdSettings} selectedPlacedHoldIds={state.selectedPlacedHoldIds} onSelectPlacedHold={state.handleSelectPlacedHold} onContextMenu={(t, i, x, y, wx, wy) => state.setContextMenu({ type: t, id: i, x, y, wallX: wx, wallY: wy })} onWallPointerUpdate={(i) => cursorPosRef.current = i} onHoldDrag={(id, x, y, segId) => { setHolds(prev => prev.map(h => (h && h.id === id) ? { ...h, x, y, segmentId: segId } : h)); state.setIsDirty(true); }} onHoldDragEnd={logic.saveToHistory} screenshotRef={screenshotRef} />
        </div>
      </div>
      <ContextMenu data={state.contextMenu} onClose={() => state.setContextMenu(null)} onUpdateData={state.setContextMenu} onCopyHold={() => { const ids = (state.contextMenu?.id && state.selectedPlacedHoldIds.includes(state.contextMenu.id)) ? state.selectedPlacedHoldIds : (state.contextMenu?.id ? [state.contextMenu.id] : []); if (ids.length) state.setClipboard(JSON.parse(JSON.stringify(holds.filter(h => h && ids.includes(h.id))))); }} hasClipboard={state.clipboard.length > 0} onPasteHold={(t) => logic.handlePaste(t)} onDelete={(id, type) => type === 'HOLD' ? logic.removeHoldsAction([id], true) : logic.removeSegmentAction(id)} onRotateHold={(id, d) => { logic.saveToHistory(); setHolds(h => h.map(x => (x && x.id === id) ? { ...x, spin: x.spin + d } : x)); }} onColorHold={(id, c) => { logic.saveToHistory(); setHolds(h => h.map(x => (x && x.id === id) ? { ...x, color: c } : x)); }} onSegmentUpdate={logic.updateSegmentQuickly} />
      <input type="file" ref={fileInputRef} className="hidden" accept=".json" onChange={(e) => { const file = e.target.files?.[0]; if (file) { logic.handleImportFile(file); e.target.value = ''; } }} />
      <GlobalModal 
        ...
        wallName={metadata.name} onWallNameChange={(n) => { setMetadata(p => ({ ...p, name: n })); state.setIsDirty(true); }}
        metadata={metadata} onMetadataChange={(m) => { setMetadata(p => ({ ...p, ...m })); state.setIsDirty(true); }}
      />
    </div>
  );
};
