...
export const ProjectsPage: React.FC = () => {
    ...
    const handleFinalDelete = async () => {
        ...
        // Vérification de sécurité : on tente une re-connexion avec l'email actuel et le code (mot de passe) fourni
        ...
        // Si l'authentification réussit, on procède à la suppression
    ...
    const handleToggleVisibility = async (e: React.MouseEvent, id: string, currentStatus: boolean) => {
        ...
        setProjects(prev => prev.map(p => 
            p.id === id ? { ...p, is_public: newStatus, data: { ...p.data, metadata: { ...p.data.metadata, isPublic: newStatus } } } : p
        ));
            ...
            <main className="max-w-7xl mx-auto px-6 py-12">
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-32 text-gray-600">
                        ...
                        <span className="font-mono text-xs uppercase tracking-widest animate-pulse">Chargement de vos fichiers...</span>
                    </div>
                ) : projects.length === 0 ? (
                            ...
                            return (
                                <div key={p.id} className="relative group">
                                    ...
                                    <div className="absolute top-3 left-3 flex gap-2 pointer-events-none z-10">
                                        {isPublic ? (
                                            <div className="px-2 py-1 bg-purple-500/90 text-white text-[10px] font-black rounded flex items-center gap-1 backdrop-blur-md shadow-lg">
                                                <Globe size={10} />
                                                <span>PUBLIÉ</span>
                                            </div>
                                        ) : (
                                            <div className="px-2 py-1 bg-gray-800/90 text-gray-300 text-[10px] font-black rounded flex items-center gap-1 backdrop-blur-md shadow-lg border border-white/10">
                                                <Lock size={10} />
                                                <span>PRIVÉ</span>
                                            </div>
                                        )}
                                    ...
                                    <div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                                        <button 
                                            onClick={(e) => handleToggleVisibility(e, p.id, isPublic)}
                                            className={`p-2 rounded-lg backdrop-blur-md border border-white/10 shadow-xl transition-all ${isPublic ? 'bg-purple-600/80 hover:bg-gray-800 text-white' : 'bg-gray-900/80 hover:bg-purple-600 text-gray-400 hover:text-white'}`}
                                        ...
                                        <button 
                                            onClick={(e) => { e.stopPropagation(); navigate(`/builder?id=${p.id}`); }}
                                            className="p-2 bg-gray-900/80 hover:bg-blue-600 text-white rounded-lg backdrop-blur-md border border-white/10 shadow-xl"
                                            title="Editer"
                                        >
                                            <Edit3 size={14} />
                                        </button>
                                        <button 
                                            ...
                                            className="p-2 bg-gray-900/80 hover:bg-red-600 text-white rounded-lg backdrop-blur-md border border-white/10 shadow-xl"
                                            title="Supprimer"
                                        >
                                            <Trash2 size={14} />
                                        </button>
                        ...
                        <div className="p-8">
                            ...
                            <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-4 mb-8">
                                <p className="text-sm text-red-200 leading-relaxed">
                                    Vous êtes sur le point de supprimer définitivement <span className="font-bold text-white">"{deleteModalTarget.name}"</span>. 
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input 
                                        ...
                                        placeholder="delete_wall"
                                        className="w-full bg-black/50 border border-gray-700 rounded-xl py-3 px-4 text-white text-sm focus:border-red-500 outline-none transition-all font-mono"
                                    />
                                </div>
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">
                                        Étape 2 : Mot de passe du compte
                                    </label>
                                    <div className="relative">
                                        ...
                                        <input 
                                            ...
                                            className="w-full bg-black/50 border border-gray-700 rounded-xl py-3 pl-10 pr-4 text-white text-sm focus:border-red-500 outline-none transition-all font-mono"
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-col gap-3 pt-4">
                                    <button 
                                        ...
                                        disabled={isDeleting || confirmKeyword !== 'delete_wall' || !confirmCode}
                                        className="w-full py-4 bg-red-600 hover:bg-red-500 disabled:bg-gray-800 disabled:text-gray-600 text-white font-black rounded-2xl transition-all shadow-xl shadow-red-900/20 flex items-center justify-center gap-3"
                                    >
