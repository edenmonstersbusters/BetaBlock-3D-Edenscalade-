    ...
    <div className="min-h-screen bg-gray-950 text-white font-sans overflow-y-auto custom-scrollbar flex flex-col">
          ...
          <div className="flex items-center gap-4">
            {user ? (
                <div className="flex items-center gap-3">
                    <button onClick={() => navigate('/projects')} className="hidden md:flex items-center gap-2 px-4 py-2 bg-gray-900 hover:bg-gray-800 border border-white/5 rounded-full text-xs font-bold transition-all text-gray-400 hover:text-white"><LayoutGrid size={14} /><span>Mes Murs</span></button>
                    <button onClick={() => navigate('/profile')} className="flex items-center gap-2 p-1 pr-4 bg-gray-900/60 hover:bg-gray-800 border border-white/5 rounded-full transition-all group backdrop-blur-md">
                        <UserAvatar url={user.user_metadata?.avatar_url} name={user.user_metadata?.display_name || user.email} size="sm" />
                        <span className="hidden sm:inline-block font-bold text-gray-300 group-hover:text-white truncate max-w-[150px]">{user.user_metadata?.display_name || user.email?.split('@')[0]}</span>
                    </button>
                    <button onClick={() => auth.signOut()} className="p-2 bg-gray-800 hover:bg-red-900/30 text-gray-400 hover:text-red-400 rounded-lg transition-colors"><LogOut size={18} /></button>
                </div>
            ) : (
                <button onClick={() => setShowAuthModal(true)} className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 rounded-full text-sm font-bold transition-all"><LogIn size={16} /><span>Connexion</span></button>
            )}
          </div>
      </div>
      ...
      <header className="relative py-24 px-6 border-b border-white/10 overflow-hidden shrink-0">
        ...
        <div className="max-w-7xl mx-auto relative z-10 text-center">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <button onClick={() => navigate('/builder')} className="group flex items-center gap-3 px-8 py-4 bg-blue-600 hover:bg-blue-500 text-white font-bold text-lg rounded-full transition-all shadow-lg shadow-blue-600/20"><Plus size={20} /><span>Créer un Nouveau Mur</span></button>
                {user && (
                    <button onClick={() => navigate('/projects')} className="flex items-center gap-3 px-8 py-4 bg-gray-900 hover:bg-gray-800 text-white font-bold text-lg rounded-full border border-white/10 transition-all"><LayoutGrid size={20} /><span>Mes Murs Privés</span></button>
                )}
            </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 flex-1 w-full">
        <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-4">
            <div className="flex items-center gap-3 text-2xl font-black text-white tracking-tight self-start md:self-auto"><Globe className="text-blue-500" size={24} /><span>Galerie</span></div>
            <form onSubmit={handleSearch} className="relative w-full md:w-64 group">
                ...
                <input type="text" value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); if (e.target.value === '') resetSearch(); }} placeholder="Rechercher..." className="w-full bg-gray-900 border border-white/10 rounded-lg py-2 pl-9 pr-8 text-sm text-white placeholder-gray-600 focus:border-blue-500 focus:bg-gray-800 outline-none transition-all" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {walls.map(wall => (
                    <WallCard 
                        ...
                        authorId={wall.user_id || wall.data?.metadata?.authorId} // On passe l'ID pour le live fetch
      ...
      <footer className="border-t border-white/5 bg-gray-950 py-12 mt-auto">
          <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6 text-gray-500 text-sm">
              <div className="flex items-center gap-2"><Database size={16} /><span className="font-mono">v1.1 Stable</span></div>
              <div className="font-mono opacity-50">© 2025 BetaBlock. Open Source Climbing Engine.</div>
          </div>
      </footer>
    </div>
  );
