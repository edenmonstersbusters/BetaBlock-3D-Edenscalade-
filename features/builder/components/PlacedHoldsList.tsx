              ...
              <div className="space-y-2 mt-4">
                {isPickingAllColor ? (
                  <div className="bg-gray-800 p-4 rounded-xl border-2 border-blue-500 shadow-2xl space-y-4 animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <ColorPalette onSelect={(c) => { onGlobalColor(c); setIsPickingAllColor(false); }} />
                  </div>
                ) : (
                  <button onClick={() => setIsPickingAllColor(true)} className="w-full py-2 px-4 bg-blue-600/20 hover:bg-blue-600/40 text-blue-400 border border-blue-600/50 rounded-lg flex items-center justify-center gap-2 text-xs font-bold transition-all">
                    <Palette size={14} /><span>Couleur globale</span>
                  </button>
                )}
                
                <button onClick={onRemoveAll} className="w-full py-2 px-4 bg-red-900/20 hover:bg-red-900/40 text-red-400 border border-red-900/50 rounded-lg flex items-center justify-center gap-2 text-xs font-bold transition-all">
                  <Trash2 size={14} /><span>Vider le mur</span>
                </button>
              </div>
            )}
        </section>
